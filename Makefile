...
SRC_ALL = $(SRC_C) src/z80.h $(Z80_TEST_C) $(SINGLE_STEP_C) test/json.h test/test.h
...
all: $(BUILD)/z80_test $(BUILD)/z80_single_step_test
...
test-all: test test-single-step
...
lint:
	$(CLANG_TIDY) $(SRC_C) $(Z80_TEST_C) $(SINGLE_STEP_C) -- $(CFLAGS) -Isrc -Itest

clean:
	rm -rf $(BUILD)

.PHONY: all test test-single-step test-all format format-check lint clean
