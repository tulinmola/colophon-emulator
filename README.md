...
There is nothing to play yet. What exists today is the first chip — a cycle-stepped Z80, complete, every instruction the machine knows, undocumented ones included — and the CPC taking shape around it: the memory map, RAM banking and ROM paging as the PAL and the Gate Array define them, and a 6845 CRTC counting out its frame at one character per microsecond. The Gate Array and the video path between them come next and come the same way: documentation first, tests as proof.
...
- ["The Amstrad CPC CRTC Compendium" v1.10](https://shaker.logonsystem.eu/ACCC1.10-EN.pdf) (Longshot / Logon System) — the CRTC as the demoscene proved it on real machines: the counters and the names it asks emulator authors to adopt, the frame construction, the two video pointers and their reload rules, the five types and their register files. Our `crtc.c` is built on it. Technical information sourced from the "Amstrad CPC CRTC Compendium" by Longshot (CC BY-NC-ND).
- ["The CRTC"](https://www.grimware.org/doku.php/documentations/devices/crtc) (Grim) — the register overview, the five types, and the board's wiring of the CRTC bus: A14 selects the chip, RS is A8, R/W is A9. The machine's four CRTC ports follow it.
- ["The Gate Array"](https://www.grimware.org/doku.php/documentations/devices/gatearray) (Grim) — the Gate Array and the PAL as their programmers knew them: the &7Fxx command dispatch, RMR's ROM enables, the eight RAM banking configurations. The machine's memory map is built from its tables.
