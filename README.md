...
```sh
make bench              # how fast this machine plays Batman Forever
```
...
- [Z80 CPU User Manual](https://www.zilog.com/docs/z80/um0080.pdf) (Zilog, UM0080) — the manufacturer's timing diagrams: the clock each machine cycle samples WAIT on, and the two wait states the processor adds to an interrupt acknowledge, of which only the second samples it — which is why an interrupt costs a CPC five microseconds rather than six.
...
- [The µPD765A/µPD765B Floppy Disc Controller](https://cpctech.cpcwiki.de/docs/upd765a/necfdc.htm) (NEC, mirrored by cpctech) — the controller's own datasheet, and everything `upd765.c` is built from: the command set and its byte counts, the three phases, the main status register's handshake, ST0 to ST3 bit by bit, the 77-step limit of Recalibrate, the polling of READY between commands, and the rule that every timing given for an 8MHz clock doubles at the 4MHz these machines supplied. It also names the shape of the disc itself, IBM System 34 double density — which is why `floppy.h` describes a medium belonging to a family of controllers rather than to a machine, and why it is the datasheet and not the image definition that settles where the control mark sits.
- ["Floppy disc controller and Floppy disc drives"](https://cpctech.cpcwiki.de/docs/fdc.html) (Kevin Thacker) — the interface as the board built it: a µPD765A at 4MHz with its interrupt, DMA and terminal count lines left unconnected, the motor driven from a port of its own to every drive at once, the Shugart lines a drive must answer, and the spin-up time nobody specifies. Our `cpc.c` decodes the interface by it and `drive.c` answers the lines it lists.
- [The AMSDOS ROM, disassembled](https://cpctech.cpcwiki.de/docs/amsdos.asm) (Kevin Thacker) — the operating system as the judge of the controller: it reads one sector at a time with EOT set to R, takes success from a normal end or an abnormal one carrying EN, drains interrupts with Sense Interrupt Status until one comes back invalid, and programs a 6ms step rate in the datasheet's units while documenting 12 — which is how the 4MHz doubling was confirmed. `upd765.c` and its tests follow the listing.
- ["Further EDSK extensions"](https://simonowen.com/misc/extextdsk.txt) (Simon Owen) — the size code as the chip really counts it, eight and above meaning 32K, correcting the earlier three-bit rule; the Offset-Info block recording where each sector lay from the index; and the gap bytes some protections hide data in. `floppy.c` and `dsk.c` follow it.
- MAME's [`upd765.cpp`](https://github.com/mamedev/mame/blob/master/src/devices/machine/upd765.cpp), [`flopimg.cpp`](https://github.com/mamedev/mame/blob/master/src/lib/formats/flopimg.cpp) and [`dsk_dsk.cpp`](https://github.com/mamedev/mame/blob/master/src/lib/formats/dsk_dsk.cpp), and Arnold's [`fdc.c`](https://github.com/rofl0r/arnold/blob/master/src/cpc/fdc.c) (Kevin Thacker) — two independent controllers agreeing on what the datasheet leaves open: a read ending at EOT without terminal count leaves C, H, R and N as they were, and a change of READY reports as interrupt code 11 with the unit and NR. MAME's track builder gives the System 34 layout in bytes and fill values, and its revolution of 100,000 cells at 300 rpm, from which `floppy.c` lays out a track; Arnold's table of sector sizes, measured on a chip, agrees with Owen on the size code.
