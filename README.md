...
There is nothing to play yet. What exists today is the first chip — a cycle-stepped Z80, complete, every instruction the machine knows, undocumented ones included — and the first piece of the CPC around it: the memory map, RAM banking and ROM paging as the PAL and the Gate Array define them. The rest of the machine, CRTC and Gate Array and the video path between them, comes next and comes the same way: documentation first, tests as proof.
