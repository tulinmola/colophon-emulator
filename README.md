...
There is nothing to play yet. What exists today is the first chip — a cycle-stepped Z80, complete, every instruction the machine knows, undocumented ones included — and the CPC taking shape around it: the memory map, RAM banking and ROM paging as the PAL and the Gate Array define them, a 6845 CRTC counting out its frame at one character per microsecond, and the Gate Array's registers and interrupt generator raising the machine's 300Hz heartbeat from it. The video path between them comes next and comes the same way: documentation first, tests as proof.
...
- ["The Gate Array"](https://www.grimware.org/doku.php/documentations/devices/gatearray) (Grim) — the Gate Array and the PAL as their programmers knew them: the &7Fxx command dispatch, the PENR/INKR/RMR layouts, the eight RAM banking configurations, mode changes taking effect after the HSYNC. The machine's memory map and the Gate Array's registers follow it.
