...
And the **duration of seventy-odd instructions** in microseconds, against two tables of measurements made independently of each other and of us — with, for two of them, the microsecond inside their own run on which the write reaches the video chip, which no duration can say and which every trick that moves a register on the character it is read at depends on; and the cost of taking an interrupt, set against the documentation's own comparison with the RST instruction that does the same thing from code. Half of that comparison agrees and half does not: the instruction costs what it should, the interrupt a microsecond more, and the test records the disagreement rather than papering over it, because nothing here can yet say which of the three numbers involved is the wrong one.
