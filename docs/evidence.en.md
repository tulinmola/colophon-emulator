...
What each group said is now set against a copy kept on record, so a change to the emulator that moves Shaker's answer moves a line of a file a human has to read before keeping it — which pins the ungraded groups before anyone can grade them. Six groups grade themselves today, in thirty-six lines, and the machine is wrong in six of them — five of those a single group's, where five instructions are measured at #CC against silicon's #C4 and nothing yet says what the difference is made of. Two more grade themselves only while they disagree, and have fallen silent. There are two such copies, one for each CRTC the tier has been run as. The second was taken from a machine that a program names a type 1 and that keeps the frame sync its own way, divides an interlaced row its own way, turns its frame parity over at every frame head whatever a program asks of it, takes both its parities from a write of the interlace register, and behaves as a type 0 in everything else, so the suite runs seventy groups at it rather than sixty-four: five of them grade themselves, in twenty-seven lines, and sixteen of those agree. Eleven disagree, which makes that record a list of the work rather than a report of it. That last rule took the count from forty-two lines to twenty-seven, the whole of a group and every one of them a disagreement, so the share rose from three eighths to three fifths on the same sixteen agreements. So the tier passing means one thing only: that nothing moved. What those records show is still the question the four CRTC types whose behaviour is not here are waiting on, and what will decide the two rules still wanting a finer instrument than one sample per character: where inside a microsecond an interrupt is raised, and where inside one a change of video mode takes hold.
