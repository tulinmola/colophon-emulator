...
Not implemented in the 6845: interlace and skew (R8 is stored and never read), the cursor, the light pen, and the states this type settles at the head of a line and acts on later. This one compares afresh at every character instead, which agrees wherever no register moves mid-line and parts company the moment one does. Two of the border's rules need a finer instrument than this one — the chip is stepped once per character, and both of those toggle display enable *inside* a character.
