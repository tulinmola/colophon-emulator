...
Not implemented in the 6845: interlace and skew (R8 is stored and never read), the cursor, the light pen, and most of the states this type settles at the head of a line and acts on later. One is here: whether the frame's last line is followed by extra ones is settled where the chip settles it, at the third character of the line rather than the first, so that a program cancelling those lines in time is obeyed. Every other comparison is made afresh at each character, which agrees wherever no register moves mid-line and parts company the moment one does. Two of the border's rules need a finer instrument than this one — the chip is stepped once per character, and both of those toggle display enable *inside* a character.
