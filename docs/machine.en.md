...
The parity the frames alternate in has no defined start: silicon wakes holding whatever it holds, and this chip wakes even. Every frame after inherits that choice, so a program that reads the parity rather than setting it can be a frame out here and right on a real machine. The frame sync's own permission, described below, has no defined start either, and this chip wakes holding it, which is the state a chip that has been running is in.
...
A line of no characters at all freezes this chip's counters nowhere, where a real one stops them where they stand — so a frame sync begun on such a line ends here on its own count, and on a real machine would not end at all.

Not implemented in the 6845: the cursor, the light pen, neither of whose pins this machine wires, and some of the states this type settles at the head of a line and acts on later. Two are here: whether the frame's last line is followed by extra ones is settled where the chip settles it, at the third character of the line rather than the first, so that a program is obeyed whether it asks for those lines in time or takes them back. A register moved at the second character can also turn that last line into an extra one itself, which is the chip's own way of adding a line without asking for any. The other is the frame sync's own permission, raised at the third character of every line and spent at the head of the next: a line too short to reach that character costs the following line its frame sync, and spends the comparison that would have raised it, so widening the line again does not bring it back. A program that shortens a line to two characters — which demos do, building a screen out of lines a microsecond or two wide — loses a frame sync it never meant to, and one that sets the sync register to the row it is already on gets a frame sync only if the line has passed its second character; at the head of a line it gets the block instead. Every other comparison is made afresh at each character, which agrees wherever no register moves mid-line and parts company the moment one does. Two of the border's rules move the display enable *inside* a character. One puts a byte of border at the end of any line whose width register was never reached, unless a delay is programmed, which carries that border to a whole character of its own instead. The other alternates shown and bordered bytes along a frame's first line, where the row register is zero. The chip is still stepped once per character; what it reports for each of them is that signal as each of the character's two bytes finds it, which is as fine as those two rules ever ask for.
